...
def process_sound_path(args):
    sound_path, speakers_md, librispeech_dir = args
    ...
    return [spk_id, sex, subset, length, rel_path]
...
def create_librispeech_dataframe(librispeech_dir, subdir, speakers_md):
    ...
    with Pool(processes=16) as pool:
        results = list(tqdm.tqdm(pool.imap(process_sound_path, [(sound_path, speakers_md, librispeech_dir) for sound_path in sound_paths]), total=len(sound_paths)))
    for result in results:
        dir_md.loc[len(dir_md)] = result
